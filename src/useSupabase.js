...
const fromFabric    = r => ({ id:r.id, name:r.name, code:r.code||"", type:r.type, color:r.color||"", widthCm:r.width_cm||0, consumptionKg:r.consumption_kg||0, unit:r.unit||"m", costPerUnit:r.cost_per_unit||0, minQty:r.min_qty||0, supplier:r.supplier||"" })
const fromAccessory = r => ({ id:r.id, name:r.name, unit:r.unit||"pcs", costPerUnit:r.cost_per_unit||0, minQty:r.min_qty||0, supplier:r.supplier||"" })
const fromSupplier  = r => ({ id:r.id, name:r.name, contact:r.contact||"", email:r.email||"", address:r.address||"", creditDays:r.credit_days||30, category:r.category||"Fabric" })
const fromPrintType = r => ({ id:r.id, name:r.name, costPerUnit:r.cost_per_unit||0 })
const fromPattern   = r => ({ id:r.id, name:r.name, styleCode:r.style_code||"", bomVersion:r.bom_version||1, sizeSet:r.size_set||"", category:r.category||"Tops", fabricId:r.fabric_id||"", fabricPerUnit:r.fabric_per_unit||0, accessories:r.accessories||[], laborCut:r.labor_cut||0, laborSew:r.labor_sew||0, laborQC:r.labor_qc||0 })
const fromOrder     = r => ({ id:r.id, orderNo:r.order_no||r.id, customer:r.customer||"", customerAddress:r.customer_address||"", patternId:r.pattern_id||"", printTypeId:r.print_type_id||"PT001", qty:r.qty||0, targetPrice:r.target_price||0, totalAmount:r.total_amount||0, status:r.status||"draft", priority:r.priority||"normal", deliveryChannel:r.delivery_channel||"", date:r.date||"", dueDate:r.due_date||"", slots:r.slots||[], specialNotice:r.special_notice||"" })
const fromBill      = r => ({ id:r.id, invoiceNo:r.invoice_no||"", supplier:r.supplier||"", date:r.date||"", status:r.status||"pending", items:r.items||[], total:r.total||0 })
...
const toFabric    = f  => ({ id:f.id, name:f.name, code:f.code||"", type:f.type||"Knit", color:f.color||"", width_cm:f.widthCm||0, consumption_kg:f.consumptionKg||0, unit:f.unit||"m", cost_per_unit:parseFloat(f.costPerUnit)||0, min_qty:parseFloat(f.minQty)||0, supplier:f.supplier||"" })
const toAccessory = a  => ({ id:a.id, name:a.name, unit:a.unit||"pcs", cost_per_unit:parseFloat(a.costPerUnit)||0, min_qty:parseFloat(a.minQty)||0, supplier:a.supplier||"" })
const toSupplier  = s  => ({ id:s.id, name:s.name, contact:s.contact||"", email:s.email||"", address:s.address||"", credit_days:parseInt(s.creditDays)||30, category:s.category||"Fabric" })
const toPrintType = pt => ({ id:pt.id, name:pt.name, cost_per_unit:parseFloat(pt.costPerUnit)||0 })
const toPattern   = p  => ({ id:p.id, name:p.name, style_code:p.styleCode||"", bom_version:parseInt(p.bomVersion)||1, size_set:p.sizeSet||"", category:p.category||"Tops", fabric_id:p.fabricId||"", fabric_per_unit:parseFloat(p.fabricPerUnit)||0, accessories:p.accessories||[], labor_cut:parseFloat(p.laborCut)||0, labor_sew:parseFloat(p.laborSew)||0, labor_qc:parseFloat(p.laborQC)||0 })
const toOrder     = o  => ({ id:o.id, order_no:o.orderNo||o.id, customer:o.customer||"", customer_address:o.customerAddress||"", pattern_id:o.patternId||"", print_type_id:o.printTypeId||"PT001", qty:o.qty||0, target_price:parseFloat(o.targetPrice)||0, total_amount:parseFloat(o.totalAmount)||0, status:o.status||"draft", priority:o.priority||"normal", delivery_channel:o.deliveryChannel||"", date:o.date||"", due_date:o.dueDate||null, slots:o.slots||[], special_notice:o.specialNotice||"" })
const toBill      = b  => ({ id:b.id, invoice_no:b.invoiceNo||"", supplier:b.supplier||"", date:b.date||"", status:b.status||"pending", items:b.items||[], total:b.total||0 })
